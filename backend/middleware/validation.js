...
export const validateEmployee = (req, res, next) => {
    ...
    if (!name || !position || !salary) {
      return res.status(400).json({ message: 'Name, position, and salary are required fields' });
    }
  ...
  export const validateInventory = (req, res, next) => {
    ...
    if (!name || currentStock === undefined || reorderLevel === undefined) {
      return res.status(400).json({ message: 'Name, current stock, and reorder level are required fields' });
    }
  ...
  export const validateSale = (req, res, next) => {
    ...
    if (!product || !quantity || !price) {
      return res.status(400).json({ message: 'Product, quantity, and price are required fields' });
    }
