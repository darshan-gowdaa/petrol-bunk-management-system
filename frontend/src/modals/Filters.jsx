...
const Filters = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center animate-modalFadeIn">
      ...
      <div className="relative w-full max-w-3xl bg-gray-900 rounded-lg shadow-xl animate-modalSlideIn">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
          <h3 className="text-xl font-medium text-white">{title}</h3>
          <button onClick={() => setShowFilters(false)} className="text-gray-400 transition-transform duration-200 hover:text-white hover:rotate-90" aria-label="Close">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="p-6">
        ...
        <div className="flex justify-end px-6 py-4 bg-gray-800 border-t border-gray-700">
          <button onClick={() => { resetFilters(); setShowFilters(false); }} className="px-4 py-2 mr-2 text-gray-300 transition-all bg-gray-700 rounded-md hover:bg-gray-600 hover:scale-105 active:scale-95 focus:outline-none focus:ring-2 focus:ring-gray-500">
            Reset
          </button>
          <button onClick={() => { applyFilters(); setShowFilters(false); }} className="px-4 py-2 text-white transition-all bg-blue-600 rounded-md hover:bg-blue-700 hover:scale-105 active:scale-95 focus:outline-none focus:ring-2 focus:ring-blue-500">
            Apply Filters
          </button>
        </div>
      </div>
